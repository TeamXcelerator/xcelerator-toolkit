...
| | • **Weil-eigenvector (ξ) cache keys on `force_even`.** The forced-even variant keeps the historical filename (`weil_eigvec_lambda_sq{L}_nmodes{N}_prec{P}.json`) with **no suffix** — so every pre-existing public fixture stays valid and the default path is unchanged. The natural variant gets a `_natural` marker (`..._prec{P}_natural.json`), so forced and natural ξ never collide on disk or in the remote cache. `load`/`save`/`fetch_remote_zip` all thread the flag through. |
