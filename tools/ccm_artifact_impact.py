...
CURRENT_IDENTITIES = {
    ...
    "ccm_sector_spectrum": "ccm-parity-sector-spectrum-v0.15.0-v1",
    "ccm_eigenfunction_profile": "ccm-eigenfunction-profile-lossless-v0.15.0-v1",
    "ccm_target_distance": "ccm-runtime-target-distance-lossless-v0.15.0-v1",
    "prolate_eigenvalue_spectrum": "prolate-fd-exact-source-spectrum-v0.15.0-v1",
    **{kind: f"{kind}-retained-parents-v0.15.0-v1" for kind in (
        "ccm_distance_resolution_evidence", "ccm_target_residual_analysis", "ccm_deviation_decomposition",
    )},
}
...
def inventory(shards: list[Path], verify_manifest_bytes: bool = False, registries: list[Path] | None = None) -> dict[str, Any]:
    ...
    for digest, record in records.items():
        ...
        if current is not None and record["mathematical_semantics_version"] != current:
    ...
    return {
        ...
        "upgrade_interpretation": "Identity changes require fresh computations when requested, not deletion or a finding of byte corruption. Unknown dependency coverage can undercount descendants. Runtime cost depends on each dimension and precision; artifact counts are not timings.",
        ...
        "coverage_complete_for_supplied_shards": not metadata_errors and not missing_active and not missing_dependencies and not missing_repositories,
