...
impl CcmParams {
    pub fn from_lambda(lambda: f64, n_modes: usize) -> Self {
        Self { lambda_squared: lambda * lambda, n_modes }
    }
...
/// Returns pairs `(n, log p)`.
pub fn prime_powers_up_to(bound: u64) -> Vec<(u64, f64)> {
...
pub fn run_f64(params: &CcmParams) -> Result<CcmResult> {
    ...
    let lambda_sq_int = (params.lambda_squared + LAMBDA_SQ_ROUNDING_EPS).floor() as u64;

    let tau = build_tau_f64(params, l, lambda_sq_int)?;
    ...
    let eigenvalues_pos = solve_spectrum(&xi, n, l, DEFAULT_BISECT_TOL, DEFAULT_BISECT_MAX_ITER)?;
    ...
    for n in -(n_max as i64)..=(n_max as i64) {
        for m in -(n_max as i64)..=(n_max as i64) {
            ...
            let integrand = |x: f64| -> f64 {
                let num = (x / 2.0).exp() * omega(x) - omega_0;
                let den = x.exp() - (-x).exp();
            ...
            let integral = xc_numerics::quadrature::gauss_legendre_64(integrand, 0.0, l);
            let wr = (omega_0 / 2.0) * kappa + integral;

            // W_p
            let mut wp_sum = 0.0_f64;
            for &(k, log_p) in &prime_powers {
                let y = (k as f64).ln();
                let q = if n == m {
...
pub fn solve_spectrum(xi: &[f64], n_max: usize, l: f64, tol: f64, max_iter: usize) -> Result<Vec<f64>> {
    ...
    for k in 1..=n_max {
        ...
        if let Some(t) = xc_numerics::root_finding::bisect(&f, a, b, tol, max_iter) {
...
mod tests {
    ...
    fn prime_powers_up_to_13() {
        ...
        let ns: Vec<u64> = pp.iter().map(|&(n, _)| n).collect();
    ...
    /// solve_spectrum with a synthetic ξ vector: constant ξ_0 = 1, all
    ...
    fn solve_spectrum_constant_xi_no_roots() {
        ...
        let roots = solve_spectrum(&xi, n_max, l, DEFAULT_BISECT_TOL, DEFAULT_BISECT_MAX_ITER).unwrap();
        // With only ξ_0 nonzero, R(t) = ξ_0 = 1 (constant), no zeros.
        assert!(roots.is_empty(), "constant R(t) should have no roots, got {:?}", roots);
    }

    /// solve_spectrum with a simple ξ that has known structure.
