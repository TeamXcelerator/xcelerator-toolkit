...
mod tests {
    ...
    fn bisect_finds_sqrt2() {
        let root = bisect(&|x| x * x - 2.0, 1.0, 2.0, 1e-15, 200).unwrap();
        let expected = std::f64::consts::SQRT_2;
        let err = (root - expected).abs();
        assert!(err < 1e-14, "bisect √2: got {}, expected {}, err {:.2e}", root, expected, err);
    }

    /// Bisect should find the root of sin(x) near π on [3, 4].
    #[test]
    fn bisect_finds_pi() {
        let root = bisect(&|x| x.sin(), 3.0, 4.0, 1e-15, 200).unwrap();
        let expected = std::f64::consts::PI;
        let err = (root - expected).abs();
        assert!(err < 1e-14, "bisect π: got {}, expected {}, err {:.2e}", root, expected, err);
    }

    /// Bisect should return None when there's no sign change.
    #[test]
    fn bisect_no_sign_change() {
        let result = bisect(&|x| x * x + 1.0, -1.0, 1.0, 1e-15, 200);
